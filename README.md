Algorithms and Data Structures — Java Exercises
This repository contains the practical exercises developed for the Algorithms and Data Structures (AED) course at ISCTE.
The exercises were implemented in Java and cover fundamental data structures, sorting algorithms, symbol tables, hashing and binary search trees.
Contents
Exercise	Topic
ex01-date	Date abstraction and object-oriented programming fundamentals
ex02-union-find	Union–Find data structure
ex03-resizing-queue	Resizing queue of strings
ex04-generic-queue	Generic queue implementation
ex05-linked-list	Linked-list implementation
ex06-ordered-list	Ordered-list implementation
ex07-improved-quicksort	Improved QuickSort algorithm
ex08-heap-sort	Heap sort and binary heaps
ex09-symbol-table	Symbol-table implementation
ex10-linear-probing-hash-table	Hash table with linear probing
extra-bst	Binary search tree exercises
exam-preparation	Additional exercises used for exam preparation


Repository structure
Each exercise is stored in its own directory and contains the corresponding Java source code:
aed-java-exercises/
├── ex01-date/
│   └── src/
├── ex02-union-find/
│   └── src/
├── ex03-resizing-queue/
│   └── src/
├── ex04-generic-queue/
│   └── src/
├── ex05-linked-list/
│   └── src/
├── ex06-ordered-list/
│   └── src/
├── ex07-improved-quicksort/
│   └── src/
├── ex08-heap-sort/
│   └── src/
├── ex09-symbol-table/
│   └── src/
├── ex10-linear-probing-hash-table/
│   └── src/
├── extra-bst/
│   └── src/
└── exam-preparation/
Technologies
- Java
- IntelliJ IDEA
- Git and GitHub
Running an exercise
1. Clone the repository:
   git clone https://github.com/USERNAME/aed-java-exercises.git
2. Open the repository in IntelliJ IDEA or another Java IDE.
3. Navigate to the relevant exercise directory.
4. Compile and run the desired class from its src directory.
Notes
- This repository is intended for academic study and to document my progress in algorithms and data structures.
- Only source files required to understand and run the exercises are included.
- Generated files and IDE-specific configuration files are excluded from version control.
Author
Luís Fernandes
